...
const TOKEN_FILE = path.join(os.homedir(), '.config', 'agent-gsuite', 'token.json');
...
export async function getAuth() {
  ...
  if (!clientId || !clientSecret) {
    throw new Error('No auth configured. Set GOOGLE_OAUTH_CLIENT_ID/SECRET env vars, or run: bun x agent-gsuite auth login');
  }
  if (!tokens) {
    throw new Error(`No tokens at ${TOKEN_FILE}. Run: bun x agent-gsuite auth login`);
  }
