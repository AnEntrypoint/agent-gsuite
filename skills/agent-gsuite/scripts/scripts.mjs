...
export async function searchScripts(auth, { query, max_results = 20 }) {
  const sc = google.script({ version: 'v1', auth });
  const drive = google.drive({ version: 'v3', auth });
  const escaped = query.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
  const q = `mimeType='application/vnd.google-apps.script' and trashed=false and name contains '${escaped}'`;
  ...
  return (r.data.files || []).map(f => ({ id: f.id, name: f.name, url: `https://script.google.com/d/${f.id}/edit`, createdTime: f.createdTime, modifiedTime: f.modifiedTime }));
}
...
export async function writeScript(auth, { sheet_id, script: identifier, file_name, mode = 'write', content, old_text, new_text, replace_all = false, file_type = 'SERVER_JS' }) {
  const data = await readScript(auth, { sheet_id, script: identifier });
  const existing = data.files.find(f => f.name === file_name);
  let files;
  if (mode === 'edit') {
    if (!existing) throw new Error(`File "${file_name}" not found`);
    const count = (existing.source.match(new RegExp(old_text.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
    ...
    const newSource = replace_all ? existing.source.split(old_text).join(new_text) : existing.source.replace(old_text, new_text);
    files = data.files.map(f => f.name === file_name ? { ...f, source: newSource } : f);
  } else {
    files = existing ? data.files.map(f => f.name === file_name ? { name: f.name, type: f.type, source: content } : f) : [...data.files, { name: file_name, type: file_type, source: content }];
  }
  const sc = google.script({ version: 'v1', auth });
